...
import { ExperimentationView } from "./components/PhaseZero/ExperimentationView";
...
const VIEWS = [
  ...
  { id: "experiment", icon: "🧪", label: "Fase 0", sub: "Experimentation" },
...
function JudgeStudio({ session, updateSession, role }) {
  ...
  const run = async () => {
    setRunning(true); await new Promise(r => setTimeout(r, 900));
    ...
    <div className="h-full flex flex-col overflow-hidden bg-gray-950">
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {filtered.map((t) => {
              const a = annotations[t.id]; const firstIn = t.inputs?.[0]?.data || {}; const meta = firstIn.metadata || firstIn; const fm = getFmTitle(a?.failure_mode_id); return (
    ...
    <div className="h-screen flex items-center justify-center bg-gray-950 text-gray-100">
      <div className="w-full max-w-md bg-gray-900 border border-gray-800 rounded-2xl p-8 shadow-xl">
        ...
        <div className="flex mb-4 text-xs bg-gray-800 rounded-lg p-0.5">
          <button
            className={`flex-1 py-1.5 rounded-md font-semibold transition-colors ${mode === "login"
          ...
          <button
            className={`flex-1 py-1.5 rounded-md font-semibold transition-colors ${mode === "register"
                ? "bg-indigo-600 text-white"
                : "text-gray-400 hover:text-gray-100"
              }`}
  ...
  return (
    <div className="h-screen flex bg-gray-950 font-sans overflow-hidden">
      ...
      <div className="flex-1 overflow-hidden">
        ...
        {view === "experiment" && <ExperimentationView />}
